...
async def start(message: types.Message):
	...
	if result[0] == 0:
		if message.chat.id == admin:
			await message.answer('Добро пожаловать, в админ-панель .', reply_markup=kb.menu)
		else:
			await message.answer('Приветствую , это бот-помощник.\nНапиши мне свой вопрос и я отправлю его администрации.')
	else:
		await message.answer('Вы были заблокированны администратором ')
...
@dp.message_handler(content_types=['text'], text='Заблокированные пользователи')
async def handlaer(message: types.Message, state: FSMContext):
	...
	if result[0] == 0:
		if message.chat.id == admin:
			...
			ids = '\n'.join(map(str, sl))
			await message.answer(f'ID заблокированных пользователей:\n{ids}')
		...
		if message.text.isdigit():
			...
			if len(result) == 0:
				...
				if id == 0:
					...
					await state.finish()
					await bot.send_message(message.text, 'Вы заблокированны администрацией')
				else:
					await message.answer('Данный пользователь уже заблокирован', reply_markup=kb.adm)
					await state.finish()
		else:
			await message.answer('Ошибка, вы вводите буквы!\nПожалуйста ведите ID пользователя')
		...
		if message.text.isdigit():
			...
			if len(result) == 0:
				...
				if id == 1:
					...
					connection.commit()
					await message.answer('Пользователь успешно разблокирован!', reply_markup=kb.adm)
					await state.finish()
					await bot.send_message(message.text, 'Вы были разблокированы администрацией.')
				else:
					await message.answer('Данный пользователь не получил пизду хахаха.', reply_markup=kb.adm)
					await state.finish()
		else:
			await message.answer('Ошибка, вы вводите буквы!\nПожалуйста ведите ID пользователя')
